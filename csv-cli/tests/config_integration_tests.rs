...
fn test_cli_creates_default_config_when_missing() {
    ...
    assert!(
        config_path.exists(),
        "Config file should be created. stderr: {}, stdout: {}",
        stderr,
        stdout
    );
    ...
    assert!(
        stderr.contains("Created default config")
            || stdout.contains("bitcoin")
            || stdout.contains("ethereum"),
        "Should create default config or show chain list. stderr: {}, stdout: {}",
        stderr,
        stdout
    );
}
...
"#;
    ...
    assert!(
        ...
        "Should not error about missing chains field. stderr: {}",
        stderr
    );
    ...
    assert!(
        ...
        "Should not have TOML parse errors. stderr: {}",
        stderr
    );
    ...
    assert!(
        stdout.contains("Chain")
            ...
            || stderr.contains("Created default"),
        "Should show chain list or create default. stdout: {}, stderr: {}",
        stdout,
        stderr
    );
}
...
fn test_cli_handles_empty_config_file() {
    ...
    assert!(
        !stderr.contains("TOML parse error"),
        "Should not error on empty config. stderr: {}",
        stderr
    );
}
...
"#;
    ...
    assert!(
        !stderr.contains("TOML parse error"),
        "Should parse valid config. stderr: {}",
        stderr
    );
    ...
    assert!(
        ...
        "Should reference bitcoin chain. stdout: {}, stderr: {}",
        stdout,
        stderr
    );
}
...
"#;
    ...
    assert!(
        !stderr.contains("TOML parse error"),
        "Should handle partial config. stderr: {}",
        stderr
    );
    ...
    assert!(
        ...
        "Should show configured chains. stdout: {}",
        stdout
    );
}
...
fn test_cli_uses_default_config_path() {
    ...
    assert!(
        config_path.exists(),
        "Default config should be created at ~/.csv/config.toml"
    );
    assert!(
        stderr.contains("Created default config"),
