...
impl IndexStorage {
    ...
    /// Store a right
    pub async fn store_right(&self, right: &IndexedRight) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// Store a transfer
    pub async fn store_transfer(&self, transfer: &IndexedTransfer) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// Get right by ID
    pub async fn get_right_by_id(&self, right_id: &Hash) -> Result<Option<IndexedRight>, Box<dyn std::error::Error>> {
        let rights = self.rights.read().await;
        Ok(rights.get(right_id).cloned())
    }
    
    /// Get transfer by hash
    pub async fn get_transfer_by_hash(&self, transfer_id: &Hash) -> Result<Option<IndexedTransfer>, Box<dyn std::error::Error>> {
    ...
    pub async fn get_chain_sync_status(&self, chain: &Chain) -> Result<Option<ChainSyncInfo>, Box<dyn std::error::Error>> {
    ...
    pub async fn log_error(
        ...
        if error_logs.len() > 1000 {
            error_logs.drain(0..error_logs.len() - 1000);
        }
    ...
    /// Get error logs
    pub async fn get_error_logs(&self, limit: Option<usize>) -> Result<Vec<ErrorLog>, Box<dyn std::error::Error>> {
