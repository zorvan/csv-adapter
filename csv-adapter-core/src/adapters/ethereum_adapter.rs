...
impl EthereumAdapter {
    ...
    pub fn capabilities() -> ChainCapabilities {
        ChainCapabilities {
            ...
            max_batch_size: 100,
            supported_networks: vec!["mainnet".to_string(), "sepolia".to_string(), "goerli".to_string()],
...
impl ChainAdapter for EthereumAdapter {
    ...
    async fn create_client(&self, _config: &ChainConfig) -> ChainResult<Box<dyn RpcClient>> {
        Err(ChainError::NotImplemented("Ethereum RPC client creation".to_string()))
    }
    
    async fn create_wallet(&self, _config: &ChainConfig) -> ChainResult<Box<dyn Wallet>> {
        Err(ChainError::NotImplemented("Ethereum wallet creation".to_string()))
    }
